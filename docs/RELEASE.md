...
- source-derived cross-service selection with no private production vocabulary,
  guarded by an independent synthetic three-service acceptance workspace and
  the committed G2–G6 regression matrix;
- Java/Spring outbound contract extraction for declarative clients and bound
  `RestClient`, `WebClient`, and `RestTemplate` receivers, including exact
  value-free Spring configuration key-group links where source proves them.
...
only candidate `d452b16`. Later source-derived generality changes require a
fresh matched matrix before entering the 1.3.0 release flow.
...
post-`d452b16` source candidate still requires a fresh matched matrix; no tag or
