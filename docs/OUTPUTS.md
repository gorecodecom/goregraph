...
- `.goregraph-workspace/workspace-map-assets/`: generated project-specific JavaScript shards used by Code Explorer. The dashboard loads a shard only after a project is selected, which keeps the initial offline document responsive. Keep this directory next to `workspace-map.html`.
- `.goregraph-workspace/workspace-map.html`: Schema 2 standalone offline dashboard with seven top-level views. Architecture derives dynamic domain lanes from `workspace-service-map.json`, keeps stable card coordinates during service/domain/direction/risk focus, groups background relationships through shared trunks, fans selected direct relationships to card ports, and keeps a persistent count/filter summary outside the SVG transform. Call badges describe statically detected relationships and are not runtime traffic metrics. Endpoints shows caller, provider, class, symbol, file, and line plus safe source actions. Feature Flow presents the route-to-component-to-API-to-backend-to-persistence-to-test chain. Data Flow shows field-level movement and explicit evidence gaps. Code Explorer exposes service-scoped classes and symbols with separate direct-usage and HTTP-reachability evidence. Diagnostics uses normal vertical HTML scrolling at 100% scale and shares canonical explanations with Query and MCP. Coverage separates workspace completeness and prioritized next scans from analyzer capability support. Every inventory-like view uses semantic HTML, and desktop or narrow layouts preserve readable controls, keyboard focus, evidence, selection ownership, and source context.
