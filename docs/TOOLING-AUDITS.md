...
`audit.areas` describes only selected indexed files. Even an area's `complete` status does not prove that all relevant repository files were indexed or that a feature is enabled. Overall source coverage remains `partial`. `audit.execution` is always `unknown`: prepared screenshot tests, documented mappings and CI job configuration do not prove approved baselines, service consumption or a successful pipeline. Binary baselines and runtime artifacts are not interpreted. The audit delivers evidence for the caller to assess; it does not automatically pronounce a setup production-ready. The separate, optional JUnit result import feeds only the human dashboard; it does not change audit or agent Context responses.
...
The new optional project `index/tooling.json` feeds the dashboard payload. It is independent of agent Context Packs; dashboard-only rebuilds preserve the existing agent projection. Dashboard revision 5 causes older dashboard projections to rebuild through the usual update command. `goregraph workspace update <root> --target dashboard` refreshes project inputs and workspace display; a dashboard refresh alone can only reuse metadata already present in project outputs.
...
## Optional JUnit results

`goregraph results import <project-root> --suite <name> --from <junit.xml-or-glob>` reads explicitly selected local reports and refreshes an existing workspace dashboard without a source rescan. For a sharded suite, pass `--expected-shards N`; absent, duplicated or mixed-time shards cannot produce an aggregate pass. GoreGraph never contacts a CI server or stores its credentials. Imported results live in `goregraph-out/test-results/results.json`, separate from `index/tooling.json` and the agent projection. A local report remains a dated local result, never a claim about the current checkout. No report is a neutral state, not a test failure. See [the command reference](../COMMANDS.md#result-import).
