...
function architectureDraftThreeWayMerge(baseDraft,localDraft,latestDraft){
  ...
  const basePositions=architectureDraftServicePositions(base),localPositions=architectureDraftServicePositions(local),changed=[],touchedGroups=new Set();architectureLocallyChangedServices(base,local).forEach(function(project){const previous=basePositions.get(project),position=localPositions.get(project);if(previous)touchedGroups.add(previous.group);if(position)touchedGroups.add(position.group);changed.push({project:project,position:position});});changed.sort(function(a,b){const aGroup=local.groups.findIndex(function(group){return group.id===a.position.group;}),bGroup=local.groups.findIndex(function(group){return group.id===b.position.group;});return aGroup-bGroup||a.position.index-b.position.index||architectureStringCompare(a.project,b.project);});
  changed.forEach(function(change){let moved=null;latest.groups.forEach(function(group){const index=group.services.findIndex(function(service){return service.project===change.project;});if(index>=0)moved=group.services.splice(index,1)[0];});moved=Object.assign({},moved||change.position.service,{manual:true});let target=latest.groups.find(function(group){return group.id===change.position.group;});if(!target){const localGroup=localGroups.get(change.position.group);target=Object.assign({},localGroup||{id:change.position.group,label:architectureDomainLabel(change.position.group)},{services:[]});latest.groups.push(target);}const localGroup=localGroups.get(change.position.group),localServices=localGroup&&localGroup.services||[],previousProjects=localServices.slice(0,change.position.index).map(function(service){return service.project;}).reverse(),nextProjects=localServices.slice(change.position.index+1).map(function(service){return service.project;}),before=nextProjects.map(function(project){return target.services.findIndex(function(service){return service.project===project;});}).find(function(index){return index>=0;}),after=previousProjects.map(function(project){return target.services.findIndex(function(service){return service.project===project;});}).find(function(index){return index>=0;}),index=before!==undefined?before:after!==undefined?after+1:Math.min(change.position.index,target.services.length);target.services.splice(index,0,moved);});
  latest.groups.forEach(function(group){if(touchedGroups.has(group.id))group.services=group.services.map(function(service){return Object.assign({},service,{manual:true});});});return latest;
}
...
function architectureDraftFromConfigData(nodes,groupRecords,config){
  ...
  domains.forEach(function(domain){const loadedGroup=configuredGroups[domain.id],detectedLabel=domain.manual?architectureDomainLabel(domain.id):domain.label;groupByID.set(domain.id,{id:domain.id,label:loadedGroup&&loadedGroup.label||detectedLabel,source:domain.source||"",confidence:domain.confidence||"",manual:!!loadedGroup,services:[]});});
  Object.keys(configuredGroups).sort(architectureStringCompare).forEach(function(groupID){if(!groupByID.has(groupID))groupByID.set(groupID,{id:groupID,label:configuredGroups[groupID].label,source:"dashboard_config",confidence:"EXACT",manual:true,services:[]});});
  (nodes||[]).forEach(function(node){const configuredService=configuredServices[node.project],groupID=configuredService?configuredService.group:architectureDomainKey(node),detectedGroup=domains.find(function(domain){return domain.id===architectureDomainKey(node);});if(!groupByID.has(groupID))groupByID.set(groupID,{id:groupID,label:(configuredGroups[groupID]&&configuredGroups[groupID].label)||architectureDomainLabel(groupID),source:configuredGroups[groupID]?"dashboard_config":node.domain_source||detectedGroup&&detectedGroup.source||"",confidence:configuredGroups[groupID]?"EXACT":node.domain_confidence||detectedGroup&&detectedGroup.confidence||"",manual:!!configuredGroups[groupID],services:[]});groupByID.get(groupID).services.push({id:node.id,project:node.project,label:node.label||node.project,order:configuredService?Number(configuredService.order)||0:Number(node.architecture_order)||0,source:configuredService?"dashboard_config":node.domain_source||detectedGroup&&detectedGroup.source||"",confidence:configuredService?"EXACT":node.domain_confidence||detectedGroup&&detectedGroup.confidence||"",manual:!!configuredService});});
...
function architectureDomains(nodes,groupRecords){
  ...
  return Array.from(groups.entries()).map(function(entry){
    ...
    return {id:id,label:record.label||architectureDomainLabel(id),order:hasOrder?Number(record.order)||0:0,hasOrder:hasOrder,source:record.source||firstNode.domain_source||"",confidence:record.confidence||firstNode.domain_confidence||"",manual:!!record.manual,color:architectureDomainColor(id),nodes:domainNodes};
