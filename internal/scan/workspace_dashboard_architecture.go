...
function architectureEditorRebase(payload,draft,previousDraft,latestDraft){
  previousDraft=previousDraft||draft;latestDraft=latestDraft||draft;const previousGroups=new Map(((previousDraft&&previousDraft.groups)||[]).map(function(group){return [group.id,group];})),latestGroups=new Map(((latestDraft&&latestDraft.groups)||[]).map(function(group){return [group.id,group];})),previousServicePositions=new Map(),latestServices=new Map();
  ((previousDraft&&previousDraft.groups)||[]).forEach(function(group){(group.services||[]).forEach(function(service,index){previousServicePositions.set(service.project,{group:group.id,index:index});});});((latestDraft&&latestDraft.groups)||[]).forEach(function(group){(group.services||[]).forEach(function(service){latestServices.set(service.project,service);});});
  const groups=((draft&&draft.groups)||[]).map(function(group){const previous=previousGroups.get(group.id),latest=latestGroups.get(group.id),services=(group.services||[]).map(function(service,index){const previousPosition=previousServicePositions.get(service.project),latestService=latestServices.get(service.project),changed=!previousPosition||previousPosition.group!==group.id||previousPosition.index!==index;return Object.assign({},service,{manual:!!(latestService&&latestService.manual)||changed});});return Object.assign({},group,{manual:!!(latest&&latest.manual)||!previous||group.label!==previous.label,services:services});});
  return {architectureRevision:payload.revision,architectureLoadedConfig:payload.config,architectureDraft:{groups:groups},architectureDirty:true};
}
