...
func TestWorkspaceDashboardAPICatalogResponsiveGeometry(t *testing.T) {
	...
	source := strings.Join([]string{
		...
		`(async()=>{const browser=await chromium.launch({headless:true,...(process.env.GOREGRAPH_PLAYWRIGHT_CHROMIUM?{executablePath:process.env.GOREGRAPH_PLAYWRIGHT_CHROMIUM}:{})}),results=[];try{for(const viewport of [{width:760,height:900},{width:1280,height:720},{width:1440,height:900},{width:1920,height:1080}]){const page=await browser.newPage({viewport});if(!page.url().includes("#token="))await page.goto("about:blank#advanced");await page.setContent(html,{waitUntil:"load"});await page.click('[data-view-mode="api-catalog"]');await page.selectOption("#api-catalog-provider-filter","service:orders|project:services/orders");await page.click(".api-catalog-endpoint summary");await page.waitForSelector(".api-catalog-details");results.push(await page.evaluate(()=>{const cells=Array.from(document.querySelectorAll(".api-catalog-endpoint-summary>span")),actions=Array.from(document.querySelectorAll(".api-catalog-trace-action,.api-catalog-details .source-actions button,.api-catalog-details .source-actions a"));return {viewport:innerWidth,scrollWidth:document.documentElement.scrollWidth,details:document.querySelectorAll(".api-catalog-endpoint[open] .api-catalog-details").length,stacked:cells.length<2||Math.abs(cells[0].getBoundingClientRect().left-cells[1].getBoundingClientRect().left)<1,actionHeights:actions.map(action=>action.getBoundingClientRect().height)};}));await page.close();}}finally{await browser.close();}process.stdout.write(JSON.stringify(results));})().catch(error=>{console.error(error);process.exit(1);});`,
	}, "\n")
	output, err := nodeScriptCommand(node, source).CombinedOutput()
	if err != nil {
