...
function toolingResults() {
  const record=workspace.results?.[selectedProject()];
  const heading='<div class="tooling-results-heading"><div><span class="detail-eyebrow">Laufnachweise</span><h2>Testergebnisse</h2></div><span>Optionaler JUnit-Import</span></div>';
  if(record?.version!==1||!record.runs?.length){
    const message=record&&record.version!==1?'Ergebnisdaten nicht verfügbar. Den Import bei Bedarf erneut ausführen.':'Kein Ergebnis importiert. Das ist bei lokal nicht ausgeführten Tests normal und sagt nichts über CI-Läufe aus.';
    return `<section class="tooling-results" aria-label="Testergebnisse">${heading}<p class="tooling-results-empty">${message}</p></section>`;
  }
  const status={passed:'Bericht bestanden',failed:'Bericht fehlgeschlagen',skipped:'Alle Tests übersprungen',incomplete:'Unvollständiger Berichtssatz'};
  const rows=record.runs.map(run=>{
    const counts=run.counts||{},reports=run.reports||[];
    const outcome=run.status==='incomplete'&&(counts.failures||counts.errors)?'Unvollständig · Fehler im Bericht':run.status==='passed'&&reports.length>1?'Berichte bestanden · Lauf nicht verifiziert':status[run.status]||'Status unbekannt';
    const provenance=run.commit?`Commit ${esc(run.commit)} (vom Importeur angegeben; nicht verifiziert)`:'Revision unbekannt';
    const origin=run.origin==='ci-artifact'?'Bereitgestelltes CI-Artefakt':'Lokale Datei';
    const shardNote=run.expected_shards?` · ${reports.length}/${esc(run.expected_shards)} Berichte`:'';
    return `<article class="tooling-run"><div class="tooling-run-title"><h3>${esc(run.suite)}</h3><span class="tooling-run-state ${run.status==='incomplete'?'is-incomplete':''}">${esc(outcome)}</span></div><p class="tooling-run-counts">${esc(counts.tests||0)} Tests · ${esc(counts.passed||0)} bestanden · ${esc(counts.failures||0)} fehlgeschlagen · ${esc(counts.errors||0)} Fehler · ${esc(counts.skipped||0)} übersprungen${shardNote}</p><p class="tooling-run-meta">${origin} · Importiert ${esc(run.imported_at||'Datum unbekannt')} · ${provenance}${run.pipeline?` · Pipeline ${esc(run.pipeline)}`:''}${run.job?` · Job ${esc(run.job)}`:''}</p><details><summary>Berichte (${reports.length})</summary><ul>${reports.map(report=>`<li><strong>${esc(report.name)}</strong><span>${esc(report.counts?.tests||0)} Tests · ${esc(report.counts?.failures||0)} fehlgeschlagen · ${esc(report.counts?.errors||0)} Fehler${report.timestamp?' · '+esc(report.timestamp):''}</span></li>`).join('')}</ul></details></article>`;
  }).join('');
  return `<section class="tooling-results" aria-label="Testergebnisse">${heading}<p class="tooling-results-note">Importierte Berichte belegen weder die Zusammengehörigkeit mehrerer Dateien noch einen erfolgreichen CI-Lauf oder den aktuellen Checkout.</p><div class="tooling-run-list">${rows}</div></section>`;
}
function renderTooling() {
  ...
  const results=toolingResults();
  if(record?.version!==1){document.getElementById('content').innerHTML=heading+results+emptyState('Tooling-Daten nicht verfügbar','Dieser Export enthält für das Projekt noch keine Tooling-Projektion. Das Dashboard mit der aktuellen GoreGraph-Version neu erzeugen.');return;}
  const sources=toolingSources(record);
  if(!sources.length){document.getElementById('content').innerHTML=heading+results+emptyState('Keine passenden Tooling-Quellen im Index','Im Snapshot wurden keine passenden Storybook-, Playwright-, Vitest- oder CI-Quellen erfasst. Das belegt nicht, dass das Projekt keine Tests hat.');return;}
