...
func BuildTestLinks(flow WorkspaceFeatureFlowRecord) []TestLinkRecord {
	...
	for _, test := range flow.Tests {
		...
		links = append(links, TestLinkRecord{ID: StableWorkspaceID("test-link", flow.ID, test.TestFile, name, relation), Relation: relation, TestFile: portableSlashPath(test.TestFile), TestName: name, Line: test.Line, Confidence: confidence, Reason: firstNonEmpty(test.Reason, "Linked from the generated test map.")})
	}
...
func BuildVerificationCommands(project WorkspaceProjectRecord, tests []TestMapRecord) []VerificationCommandRecord {
	...
	workingDirectory := portableSlashPath(project.Path)
	if len(tests) == 0 {
		return nil
	}
	commands := make([]VerificationCommandRecord, 0, len(tests))
	for _, test := range tests {
		file := portableSlashPath(test.TestFile)
...
func portableSlashPath(value string) string {
