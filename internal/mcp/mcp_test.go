...
func TestInitializeInstructsAgentsToReuseIncludedSource(t *testing.T) {
	...
	if count := strings.Count(payload.Instructions, agentguide.AssistedInstruction); count != 1 {
		t.Fatalf(
...
func TestMCPTaskContextValidatesArgumentsStrictly(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unknown mode", mutate: func(args map[string]any) { args["mode"] = "typo" }},
		{name: "wrong mode type", mutate: func(args map[string]any) { args["mode"] = true }},
		{name: "empty mode", mutate: func(args map[string]any) { args["mode"] = "" }},
