...
The current development release target is `v0.9.0`.
...
The workspace dashboard at `.goregraph-workspace/workspace-map.html` is a standalone offline UI. It opens with an Open Issues workbench that groups unresolved, mismatched, dynamic, and out-of-scope contracts by cause, including repeated `/tree/...` frontend prefix problems. It also provides an Architecture Map for domain-grouped service relationships, a Focused Service view for selected incoming/outgoing relationships, endpoint traces from frontend consumer to backend handler, and Endpoint Paths that list a selected service as caller -> endpoint/relation -> provider. Scanned frontend projects stay visible even when no outgoing API calls were detected. Nodes can be selected in the sidebar or the graph, selected again to clear, cleared with Escape or Clear selection, panned with drag, zoomed with the mouse wheel, and inspected with explained `RESOLVED`, `MISMATCH`, `UNRESOLVED`, `OUT_OF_SCOPE`, `EXTRACTED`, and `MATCHED` status terms. Endpoint and issue details include generated `file://` source links with file and line labels when scan data has source locations.
