...
- local JavaScript/TypeScript API helper and `fetch` contract extraction
...
The current public release is `v0.5.0`. The current development target is `v0.6.0`.
...
- JavaScript/TypeScript: functions, classes, imports, exports, package scripts, Express/Fastify-style routes, React Router routes, app-specific route IDs in monorepos, API helper/`fetch` contracts, calls, route flows, and test mappings.
- Java: classes, interfaces, enums, imports, Spring endpoints, Spring dependencies, Java/Spring callgraph, endpoint flows, and test mappings.
- Markdown: headings.
...
`api-contracts.json` contains JavaScript/TypeScript HTTP client usage detected from supported helpers and `fetch` calls.
