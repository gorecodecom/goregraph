...
Exact missing-transition change plans may include up to four `plan_files` with
existing provider-test or caller mock/retry-pattern identities. These entries
are compact metadata, do not increase the 12-file source boundary, and never
authorize a read. Read only a bounded range for the same path when it is also
listed in `source_omissions`; pattern entries are references, not change
targets.

The same request may include `production_plan_files` groups with one exact
existing provider-contract path and up to two exact primary-persistence paths.
...
The standard server exposes only `task_context`. Give it a focused query with
